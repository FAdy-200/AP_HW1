...
# Date : 06/12/2020
# main file to open the wanted game
...
class startingScreen:
    ...
    def _center_text(self, text, mode=None):
        ...
        if mode is None:
            ...
            # sets the center of the rectagnle to be under the center of the screen shifted to the left
            tr.center = self.width / 2 + 100, self.height / 2 + 100
        elif mode == "right":
            # sets the center of the rectagnle to be under the center of the screen shifted to the right
            tr.center = self.width / 2 - 100, self.height / 2 + 100
        return tr
        ...
        while True:  # infinitely showing the screen until the user chooses
            for event in pygame.event.get():  # getting the events that are happening on the screen
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:  # checking if the user pressed the left click on the screen
                    if 430 > self.mouse[1] > 370:  # checking if the mouse is on the vertical range of the two choices
